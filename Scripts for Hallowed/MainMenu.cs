...
namespace GreatArcStudios
{
    ...
    public class MainMenu : MonoBehaviour
    {
        ...
        private BzThirdPersonChCtrler characterController; // Reference to the character's movement script
        private Animator characterAnimator; // Reference to the character's Animator component
        ...
        /// <summary>
        /// Initialise a new game (send to GameScene)
        /// </summary>
        public void NewGame()
        {
        ...
        public void Resume()
        {
            ...
            mask.SetActive(false);
            EnableCharacterControls(); // Enable all character control scripts
        ...
        public void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Escape) && mainPanel.active == false)
            {
                ...
                mask.SetActive(true);
                Time.timeScale = 0;
                DisableCharacterControls(); // Disable all character control scripts
                for (int i = 0; i < otherUIElements.Length; i++)
                {
        ...
        {
            uiEventSystem.SetSelectedGameObject(defualtSelectedMain);
            StartCoroutine(cancelControlMain());
        }
        /// <summary>
        /// Use an IEnumerator to first play the animation and then changethe video settings
        /// </summary>
        /// <returns></returns>
        internal IEnumerator cancelControlMain()
        {
            vidPanelAnimator.Play("Video Panel Out");
